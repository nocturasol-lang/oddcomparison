...
# Recycle browser to avoid session bloat and rate limits
BROWSER_RECYCLE_SEC = 300  # 5 minutes
...
class NovibetScraper(BaseScraper):
    ...
    async def _ensure_browser(self) -> None:
        """Recycle browser if older than BROWSER_RECYCLE_SEC."""
        if self._browser is None:
            await self.initialize()
            return
        elapsed = time.monotonic() - self._browser_created_at
        if elapsed >= BROWSER_RECYCLE_SEC:
            log.info("NovibetScraper: recycling browser after %.0fs", elapsed)
            await self.cleanup()
            await self.initialize()
